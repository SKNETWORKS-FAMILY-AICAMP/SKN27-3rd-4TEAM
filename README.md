...
|PostgreSQL 스키마 설계
정형 데이터 쿼리 최적화
벡터 DB 검색 보조|CSV 데이터 전처리·적재
외부 데이터 수집 (RAG용)
정기 업데이트 자동화|챗봇 UI/UX
시각화
사용자 입력 폼|FastAPI 백엔드 서버
RAG 검색 파이프라인
LLM 호출 + 답변 생성|챗봇 UI/UX
시각화
사용자 입력 폼|사용자 의도 분석
여러 전문 에이전트 오케스트레이션
LLM 답변 종합|
...
## ⭐ 한줄회고
- 김재묵
- 김한솔 : 각자 알아서 다 잘하고 힘내줬는데 각자 도메인을 모르고 시작하여 시작부터 힘들고 아쉬웠지만 일단 공부한 내용을 더 자세히 알게되어 좋았고, 누구하나 빠짐없이 자리에서 힘냈는데 결과가 안나와서 아쉬웠지만 다들 모두 최선을 다했기에 후회는 없습니다.
- 박준희
- 박창제
- 오주희
- 주연중

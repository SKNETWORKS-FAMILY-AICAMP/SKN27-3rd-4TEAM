...
| 정형 데이터 쿼리 최적화 | 외부 데이터 수집 (RAG용) | ML·DL 분석 | RAG 검색 파이프라인 | ML·DL 분석 | 에이전트 오케스트레이션 |
| 벡터 DB 검색 보조 | 정기 업데이트 자동화 | 시각화·사용자 입력 폼 | LLM 호출 + 답변 생성 | 시각화·사용자 입력 폼 | LLM 답변 종합 |
...
#### 2. AI 챗봇 상담 흐름
![AI_Chatbot.png](images/AI_Chatbot.png)

#### 3. 그래프 빌드 (오프라인 1회)
![graph_build](images/graph_build.png)
...
## ⭐ 한줄회고
- 김재묵 : 기초 설계부터해서 틀을 확실하게 만들어놔야 나중에 하다가 막혀도 그걸 보면서 길을 찾는데, 그런 전체적인 흐름도를 보여주는 설계 부분이 부족했고 해당 주제의 도메인 지식 또한 중요하다고 생각했습니다. 그래도 아무런 문제없이 잘되는 프로젝트보다  이렇게 시행착오를 겪어가면서 깨달음을 얻어가는 프로젝트가 더 소중하고 좋은 경험이라고 느꼈습니다.

- 김한솔 : 전세가 뭔지 매매가 뭔지 임대차보호법은 또 뭔지, 코드보다 도메인 공부가 더 힘들었던 프로젝트였는데 그래도 막힌 팀원이랑 같이 코드 붙잡고 내 파트 아닌 것도 조금씩 기웃거리다 보니 어느새 다 같이 완성해 있었다. 다음 프로젝트에서는 도메인을 먼저 깊게 이해한 뒤 시퀀스 다이어그램으로 흐름을 탄탄하게 설계하고, 그 설계가 코드로 자연스럽게 이어지는 경험을 만들어보고 싶다

- 박준희 : 각자의 역할을 나누고 수행하였지만, RAG, Multi-Agent, UI 구현 등 서로 연결되는 부분에 대한 소통과 논의가 부족했다. 주제 변경으로 인한 시간 부족과 각자의 업무를 먼저 끝내야 한다는 책임감이 원인이었던 것 같다. 주제 선정 후 시퀀스 다이어그램을 먼저 그려 전체 흐름을 팀원과 공유하고 시작하는 것이 필요할 것 같다.
또한 RAG, GraphDB 등 수업에서 배운 개념들을 프로젝트에 직접 연결 지어 생각하는 것이 아직 부족하다는 것을 느꼈고, 개념 이해에서 실제 적용까지의 갭을 줄이는 것이 앞으로의 과제라고 생각한다. 화면 구현의 경우 보여주고자 하는 기능을 바탕으로 전체 큰 그림을 먼저 그리고 LLM을 활용해 초반에 최대한 많은 기능을 구현한 뒤 백엔드와 협업하며 실제로 구현 가능한 기능으로 걸러내는 방식으로 진행하는 것이 중요하다는 것을 프로젝트를 통해 배울 수 있었다.

- 박창제 : graphdb에 대해 이해가 부족하다. 왜 노드가 만들어지면 노드끼리의 관계가 뭘 의미하는지 모르고, graphdb를 왜 사용해야할지 이해를 못했다. graphdb의 사용 목적에 대해 이해하고 구현 로직에 대해 생각해봐야겠다.
그리고 구현을 해보면서 RAG의 구조를 이해할 수 있었다. 판례, 법령 등 pdf 파일을 chunk 파일로 만들기 위해 text를 뽑아내고 text 데이터를 필요한 데이터만 사용할 수 있도록 split과 cleaning 작업으로 chunk 데이터를 만들어냄 만들어진 chunk 데이터를 llm이 사용할 수 있도록 embedding 과정을 진행 해다. llm이 input 데이터와 유사한 데이터를 embedding 데이터로 찾으며 결과 데이터에 반영할 수 있도록 했고 이런 식으로 RAG의 구조에 대해 이해할 수 있었다.

- 오주희 : 많은 기능을 구현하고자 하였으나, 초반에 시퀀스 다이어그램이 정리되지 않아 전체 시스템 흐름을 파악하는 데 시간이 걸렸다. 이를 통해 설계 문서화의 중요성을 느꼈다.

- 주연중 : 전체적인 계획을 무조건 먼저 세우는것이 아주 중요하다는것을 깨달을수 있었던 귀중한 경험 이였습니다.

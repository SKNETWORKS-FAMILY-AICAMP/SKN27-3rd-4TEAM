...
- 박창제 :
 부족한점 :
RAG를 구현하며 왜 사용하는지 생각을 해보지 않고 바로 바이브 코딩을 하여 저대로된 로직 판단을 못했다.
graphdb의 riskfactor 라는 노드가 있었는데 무슨 데이터로 해당 노드를 만들었는지 이해하지 못한 상태로 사용함.


개선할 점 :
아직 graphdb에 대해 이해가 부족하다.
왜 노드가 만들어지면 노드끼리의 관계가 뭘 의미하는지 모르고, graphdb를 왜 사용해야할지 이해를 못했다.
graphdb의 사용 목적에 대해 이해하고 구현 로직에 대해 생각해봐야겠다.


잘한점 : 
구현을 해보면서 RAG의 구조를 이해할 수 있었다.
판례, 법령 등 pdf 파일을 chunk 파일로 만들기 위해 text를 뽑아냄.
text 데이터를 필요한 데이터만 사용할 수 있도록 split과 cleaning 작업으로 chunk 데이터를 만들어냄
만들어진 chunk 데이터를 llm이 사용할 수 있도록 embedding 과정을 함.
llm이 input 데이터와 유사한 데이터를 embedding 데이터로 찾으며 결과 데이터에 반영할 수 있도록 한다.
이런 식으로 RAG의 구조에 대해 이해할 수 있었다.

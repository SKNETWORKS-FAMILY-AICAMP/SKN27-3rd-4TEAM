...
- `common/agents/` 아래에는 LLM/ReAct Agent 정의, 프롬프트, tool 조합을 둔다.
- LLM 판단이 필요한 Agent는 `create_react_agent` 기반 sub-agent로 만들고 tool을 전달한다.
- Graph node는 프로젝트 전용 state를 Agent 입력/출력으로 변환하고 state를 업데이트하는 adapter 역할만 한다.
...
Agent 정의는 `common/agents/`, Tool은 `common/tools/`, Node adapter는 `common/nodes/`에 둔다.
...
| --- | --- | --- | --- |
| `special_clause_react_agent` | `common/agents/diagnosis_agents.py` | `adaptive_rag_tool` | 계약서 특약 위험과 빠진 방어 특약 판단 보조 |
| `legal_case_retriever_react_agent` | `common/agents/legal_consultation_agents.py` | `adaptive_rag_tool` | 사용자 질문과 관련된 내부 판례/판결문 근거 검색 |
| `legal_law_guide_retriever_react_agent` | `common/agents/legal_consultation_agents.py` | `adaptive_rag_tool` | 법령, 공공 가이드, 체크리스트 근거 검색 |
| `case_based_answer_react_agent` | `common/agents/legal_consultation_agents.py` | `adaptive_rag_tool`, `search_external_sources_tool` | 판례/법령/외부자료 기반 법률 정보 답변 작성 |
| `defense_roleplay_react_agent` | `common/agents/defense_simulation_agents.py` | `adaptive_rag_tool` | 전세사기 방어 RPG의 악역 NPC 압박 대사 생성 |

ReAct agent 공통 생성 로직은 `common/agents/react_agent_factory.py`에 둔다. 각 도메인별 Agent 정의와 프롬프트는 `common/agents/*_agents.py`에 둔다. Ollama tool-calling을 위해 `langchain_ollama.ChatOllama`을 우선 사용하며, `ENABLE_REACT_AGENTS=0`이면 deterministic fallback으로 실행된다.
...
common/agents/                  # 실제 Agent 정의, 프롬프트, tool 조합
  react_agent_factory.py
  diagnosis_agents.py
  legal_consultation_agents.py
  defense_simulation_agents.py
...
| `user_action_interpreter_node` | 사용자 대응을 Agent로 정규화하고 fallback rule로 방어 행동 탐지 |
...
- `common/agents`: `create_react_agent` 기반 ReAct agent factory와 도메인별 Agent 정의
- `common/tools`: `@tool` 공개 도구와 내부 helper
...
→ 서울고법/대전고법/서울중앙지법 같은 축약 법원명과 대법원/헌법재판소 단독 표기까지 추출

source_id 안정화
→ row_id를 우선 source_id로 사용
→ row_id가 없으면 file_name#chunk_index를 사용
→ 둘 다 없을 때만 rag-ref-N fallback 사용

검색 품질 보정
→ 질문 유형별 must_terms/boost_terms를 search_plan에 포함
→ vector score에 키워드 매칭 보너스, 필수어 누락 감점, 노이즈 제목 감점을 적용한 rerank_score 계산
→ reference metadata에 vector_score와 rerank_score를 함께 보존
→ doc_type 다양성 보장 시 rerank_score 0.35 미만 문서는 forced pick에서 제외
→ 최종 references 출력 순서는 다시 rerank_score 내림차순으로 정렬
```
...
| 체크리스트 DONE 판정 안전화 | 반영 | RAG 문서 근거는 안내 근거로만 사용한다. `DONE`은 항목별 화이트리스트에 해당하는 실제 확인 자료 또는 사용자 명시 체크가 있을 때만 부여한다. |

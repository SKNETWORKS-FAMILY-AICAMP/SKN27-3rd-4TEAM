...
def area_bucket_market(transactions: pd.DataFrame, info: dict[str, Any]) -> dict[str, Any]:
    ...
    low_price_anomaly = bool(jeonse_pp and contract_pp < jeonse_pp * 0.85)
    ...
    return {
        ...
        "low_price_anomaly_rule": "contract_jeonse_per_pyeong < area_bucket_jeonse_per_pyeong * 0.85",
    }
...
def final_risk_level(levels: list[str | None]) -> str | None:
    order = {
        "안전": 1,
        "주의": 2,
        "위험": 3,
    ...
    if not valid:
        return None
    return max(valid, key=lambda level: order[level])
...
def analyze_contract(contract_info: dict[str, Any], artifact_dir: Path = ARTIFACT_DIR) -> dict[str, Any]:
    ...
    support_forecast = forecast_market(row, info, best_models, SUPPORT_HORIZON)

    final_level = final_risk_level([
        risk_level(current_ratio),
        area_check.get("area_bucket_risk_level"),
        primary_forecast.get("forecast_risk_level"),
        support_forecast.get("forecast_risk_level"),
    ])
    ...
    return {
        ...
        "limitations": [
            ...
            "24개월 모델은 baseline보다 낫지만 과적합 severe 경고가 있어 단독 판단이 아니라 보조 지표로 사용해야 합니다.",
